...
var createDownloadThread = function(index) {
	...
	var onData = function(dataChunk) {
		//console.log('writeposition:', writePosition);
		if (writePosition[index] == threads.getStatus()[index].position) {
			//console.log('Thread:', threads.getStatus()[index]);
			dataReceived += dataChunk.length;
			var position = threads.getStatus()[index].position;
			threads.setPosition(index, dataChunk.length);
			writePosition[index] = threads.getStatus()[index].position;
			writer.write(dataChunk, position, _onChunkSaved);

		} else {
			console.log('Data written already!');
		}
	...
	for (var i = 0; i < _options.mirrorHosts.length; i++) {
		var hostname = _options.mirrorHosts[i];
...
module.exports = function(options) {
	...
	writePosition = {};
	var reqUrl = url.parse(_options.url);
	_options.mirrorHosts = _options.mirrorHosts || [reqUrl.hostname];
