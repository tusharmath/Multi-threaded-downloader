...
var setupTasks = function() {
	...
	this.tasks = {
		...
		'data-request': ['read-meta-data', 'threads-generator', 'file-handle-generator', function(callback, results) {
			...
			async.each(results['threads-generator'], function(item, callback) {
				...
				var writer = new DownloadWriter(results['file-handle-generator']);
				writer.callback = onWrite;
				...
				var onData = function(data) {
					...
					writer.execute(executeParams);
				};


				var dataRequest = new DataRequest(results['read-meta-data'].url, item.header, onData);
				taskInit.call(dataRequest, callback);
			}, callback);
		}],
		...
		'generate-meta-data': ['file-name-generator', 'threads-generator', function(callback, results) {
			...
			var data = {};
			data.threads = results['threads-generator'];
			data.url = self.url;
			var req = new MetaDataBuilder(file, data);
			taskInit.call(req, callback);
